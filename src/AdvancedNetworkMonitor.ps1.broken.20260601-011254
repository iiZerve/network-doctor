...
function Get-HighVolumeLoss {
    <#
    .SYNOPSIS
        Performs a large number of rapid pings to get accurate packet loss over a time window.
    #>
    param(
        [string[]]$Targets = @("8.8.8.8", "1.1.1.1"),
        [int]$TotalPings = 250,           # Aim for a few hundred checks
        [int]$TimeoutMs = 1200
    )
    ...
    $loss = if ($totalAttempts -gt 0) {
        [math]::Round( (100 * ($totalAttempts - $successes) / $totalAttempts) , 1)
    } else { 100 }
    ...
    $jitter = if ($latencies.Count -gt 1) {
        ...
        $jitterVals = $latencies | ForEach-Object { [math]::Abs(
$script:Version = "0.2.0"
<#
.SYNOPSIS
    Grok Advanced Network Doctor v2 - Full monitoring + diagnostics + optimization + support report generator.
...
while ($true) {
        ...
        $stats = Get-HighVolumeLoss -Targets @("8.8.8.8","1.1.1.1") -TotalPings 250
...
function Show-LossGraph {
    ...
    $displayHistory = if ($History.Count -gt $MaxSamples) { 
        $History[($History.Count - $MaxSamples)..($History.Count-1)] 
    } else { $History }
    
    Write-Host "`nPacket Loss % over last $($displayHistory.Count) samples (each ~30s):" -ForegroundColor Cyan
    
    $height = 10
    $barChars = ' ▁▂▃▄▅▆▇█'
    
    # Find max for scaling (at least 10%)
    $maxLoss = [math]::Max(10, ($displayHistory | ForEach-Object { 
$script:Version = "0.2.0"
<#
.SYNOPSIS
    Grok Advanced Network Doctor v2 - Full monitoring + diagnostics + optimization + support report generator.
...
while ($true) {
 ...
 - $avgLatency) }
        [math]::Round( ($jitterVals | Measure-Object -Average).Average , 1)
    } else { $null }
...
.Loss } | Measure-Object -Maximum).Maximum )
    
    # Draw from top to bottom
    for ($row = $height; $row -ge 0; $row--) {
        $threshold = ($row / $height) * $maxLoss
        $line = ""
        foreach ($entry in $displayHistory) {
            $level = [math]::Floor( ($entry.Loss / $maxLoss) * $height )
            if ($level -ge $row) {
                $line += "█"
            } else {
                $line += " "
        ...
        $label = if ($row % 2 -eq 0) { 
            ([math]::Round($threshold)).ToString().PadLeft(3) + "%" 
        } else { "    " }
    ...
    # Time labels
    $timeLine = "     "
    $step = [math]::Max(1, [math]::Floor($displayHistory.Count / 6))
    for ($i = 0; $i -lt $displayHistory.Count; $i++) {
        if ($i % $step -eq 0) {
            $t = $displayHistory[$i].Time.ToString('HH:mm')
            $timeLine += $t.PadRight($step)
        } else {
            $timeLine += " "
        }
    }
